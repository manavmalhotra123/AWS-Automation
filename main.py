...
def disable_s3_bucket_public_access(s3_client, bucket_name):
...
def check_ec2_inactivity(ec2_client, threshold_minutes, exceptions=None):
...
def check_s3_inactivity(s3_client, threshold_minutes):
...
def main():
    if len(sys.argv) != 2 or sys.argv[1] not in ("ec2", "s3"):
        print("Usage: python main.py <client>")
        print("Valid clients: ec2, s3")
        sys.exit(1)

    client_type = sys.argv[1]

    # AWS clients - currently for ec2 and s3
    ...
    while True:
        if client_type == "ec2":
            ...
            inactive_ec2_instances = check_ec2_inactivity(ec2_client, inactivity_threshold_minutes, exceptions=ec2_instances)
            ...
            inactive_s3_buckets = check_s3_inactivity(s3_client, inactivity_threshold_minutes)
            for bucket_name in inactive_s3_buckets:
                disable_s3_bucket_public_access(s3_client, bucket_name)
